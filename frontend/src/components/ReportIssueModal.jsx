...
const ReportIssueModal = ({ onClose, onSuccess }) => {
  ...
  const getCurrentLocationDetails = async () => {
    ...
    try {
      ...
      const mockAddress = `${location.latitude.toFixed(4)}, ${location.longitude.toFixed(4)}, Ahmedabad, Gujarat`
      ...
      <div className="bg-gray-800 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Photo Upload */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Add/Upload Photos (up to 3)</label>
              <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center">
                ...
                {formData.photos.length > 0 && (
                  <div className="mt-2 text-sm text-gray-400">{formData.photos.length} photo(s) selected</div>
                )}
              </div>
            </div>
            ...
            {/* Location Display */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Location (Auto-detected)</label>
              <div className="bg-gray-700 p-3 rounded-lg">
                <div className="text-sm text-gray-300">{locationData?.address || "Detecting location..."}</div>
                {location && (
                ...
                {MapView}
                <div className="mt-4 rounded-lg overflow-hidden"> {/* Added margin-top for spacing and rounded corners */}
                  <MapView />
                </div>
            ...
            <div className="flex items-center">
              ...
              />
              <label className="ml-2 text-sm text-gray-300">Report Anonymous</label>
            </div>

            {/* Error Message */}
            {error && (
              <div className="bg-red-900 border border-red-700 text-red-300 px-4 py-3 rounded-lg text-sm">{error}</div>
            )}
